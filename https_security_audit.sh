...
# HTTPS Security Audit Script - 2026 Kali Edition
...
show_help() {
    ...
    echo "  -t <target>      Single IP, Domain, or CIDR range"
    echo "  -l <file>        Path to a file containing a list of targets"
...
for target in $IP_LIST; do
    ...
    log "\n${BLUE}[*] Starting HTTPS Audit for: $target${NC}"

    if ! nmap -p 443 --open -Pn "$target" | grep -q "443/tcp open"; then
        log "${RED}[-] Port 443 closed or filtered. Skipping.${NC}"
        continue
    fi

    log "[+] Running deep scan (testssl)..."
    TESTSSL_DATA=$(testssl --quiet --color 0 -p -S -U -H "$target:443" 2>/dev/null)
    ...
    # 2. CURL HSTS Verification
    log "${BLUE}[+] Running CURL Verification for HSTS Header...${NC}"
    HSTS_HEADER=$(curl -s -I -k "https://$target" | grep -i "Strict-Transport-Security")
    ...
    # 3. Deep Nmap Probes with Forced Color Highlighting
    if echo "$FINDINGS" | grep -qiE "CBC|vulnerable|deprecated|TLSv1|3DES|obsoleted"; then
        ...
        # grep --color=always ensures the RED stays through the log pipe
        nmap -sV -p 443 --script ssl-enum-ciphers "$target" | grep --color=always -Ei "CBC|$" | sed 's/^/        /' | tee -a "$OUTPUT_FILE"

        # Probe 2: Sweet 32 / 3DES / TLS Detail
        log "    [PROBE] Enumerating 3DES and Legacy Protocols (TLS 1.0/1.1):"
        nmap -Pn -T4 -sT -p 443 --script ssl-enum-ciphers "$target" | \
        grep --color=always -Ei "3DES|TLSv1\.0|TLSv1\.1|$" | sed 's/^/        /' | tee -a "$OUTPUT_FILE"
    fi

    # 4. Final Summary
    log "${YELLOW}[!] Audit Summary for $target:${NC}"
    HAS_ISSUES=false
